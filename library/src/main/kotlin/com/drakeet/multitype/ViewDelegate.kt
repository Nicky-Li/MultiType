...
/**
 * This is a simple [ItemViewDelegate] that does not require to declare and provide a
...
abstract class ViewDelegate<T, V : View> : ItemViewDelegate<T, ViewDelegate.Holder<V>>() {
        ...
        get() {
            return holder(this)
                ?: throw IllegalAccessException(
                    "The view holder property can only be called after onCreateView()!")
        }

    protected val View.layoutPosition: Int
        get() = holder.layoutPosition

    protected val View.absoluteAdapterPosition: Int
        get() = holder.absoluteAdapterPosition

    protected val View.bindingAdapterPosition: Int
    ...
    private fun holder(view: View): Holder<V>? {
        @Suppress("UNCHECKED_CAST") return view.getTag(R.id.tagViewHolder) as? Holder<V>
    }
    ...
    override fun onBindViewHolder(holder: Holder<V>, item: T, position: Int) =
        onBindView(holder, holder.view, item, position)
