...
interface OneToManyEndpoint<T> {
    ...
    private fun withJavaClassLinker(
        classLinker: (position: Int, item: T) -> Class<out ItemViewDelegate<T, *>>
    ) {
        withJavaClassLinker(
            object : JavaClassLinker<T> {
    ...
    fun withKotlinClassLinker(
        classLinker: (position: Int, item: T) -> KClass<out ItemViewDelegate<T, *>>
    ) {
