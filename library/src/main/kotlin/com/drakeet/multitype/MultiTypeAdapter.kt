...
constructor(
    /**
     * Sets and updates the items atomically and safely. It is recommended to use this method to
     * update the items with a new wrapper list or consider using [CopyOnWriteArrayList].
     *
     * Note: If you want to refresh the list views after setting items, you should call
     * [RecyclerView.Adapter.notifyDataSetChanged] by yourself.
     *
     * @since v2.4.1
     */
    ...
    /**
     * Registers a type class and its item view delegate. If you have registered the class, it will
     * override the original delegate(s). Note that the method is non-thread-safe so that you should
     * not use it in concurrent operation.
     *
     * Note that the method should not be called after [RecyclerView.setAdapter], or you have to
     * call the setAdapter again.
     *
    ...
    /**
     * Registers a type class to multiple item view delegates. If you have registered the class, it
     * will override the original delegate(s). Note that the method is non-thread-safe so that you
     * should not use it in concurrent operation.
     *
     * Note that the method should not be called after [RecyclerView.setAdapter], or you have to
     * call the setAdapter again.
     *
    ...
    /**
     * Registers all of the contents in the specified [Types]. If you have registered a class, it
     * will override the original delegate(s). Note that the method is non-thread-safe so that you
     * should not use it in concurrent operation.
     *
     * Note that the method should not be called after [RecyclerView.setAdapter], or you have to
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int, payloads: List<Any>) {
        ...
        getOutDelegateByViewHolder(holder).onBindViewHolder(holder, item, position, payloads)
    }
     ...
     * its transient state, and passes the event to its associated item view delegate.
     *
    ...
    /**
     * Called when a view created by this adapter has been detached from its window, and passes the
     * event to its associated item view delegate.
     *
